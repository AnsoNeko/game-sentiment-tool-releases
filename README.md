...
- 版本：`v0.4.7`（普通版 Latest）
- 平台：Windows x64
- 形式：NSIS 安装程序
- 下载：[前往最新 Release](https://github.com/AnsoNeko/game-sentiment-tool-releases/releases/latest)

> v0.4.7 优化B站广泛监测：按关键词与投稿日期定向搜索，标题或真实标签核验匹配；支持综合/最新各1–100页上限和匹配列表CSV导出。数据库增量升级至v12，保留历史、排除标记及提醒去重。
...
1. 在最新 Release 页面下载 `GameSentimentTool-v0.4.7-win-x64-Setup.exe`。
2. v0.3.3-v0.4.6 用户可通过软件内更新获取 v0.4.7；本次未执行真实安装/升级测试。**v0.3.2 及更早版本必须手工下载 v0.4.7，并以管理员身份运行一次。** 旧版更新助手会在新的安装握手代码生效前自行退出，无法通过远端资源反向修复。
...
```text
741D206D3CDCC0D065CDF1826DA9C50844817FC2D4A438823BF29137209FA8D7
```

## 发布验证约定

默认执行源码测试、构建、安装包内容、版本、大小、SHA-256、更新清单及远端资产元数据校验。资产回下载、运行冒烟、真实下载安装和覆盖升级测试仅在用户明确要求时执行；本次未执行这些实机验收。历史版本的真实升级记录不代表本版本已经实测。
